...
SIDBY_conds_SERFR = {'track': (340, 346), 'alt': (4000, 5501), 'ground_speed': (200, 275),
                         'vrate': (-2000, -100), 'ground_distance': (0, 0.125), 'name': 'SERFR'}

EDDY2_conds_SERFR = {'track': (340, 346), 'alt': (5700, 8401), 'ground_speed': (105, 401),
                         'vrate': (-2053, -457), 'ground_distance': (0, 0.125), 'name': 'SERFR'}

EDDY3_conds_SERFR = {'track': (340, 346), 'alt': (4500, 7400), 'ground_speed':(70, 265),
                    'vrate':(-2176, 594), 'ground_distance': (0, 0.125), 'name': 'SERFR'}

NARWL_conds_SERFR = {'track':(340, 346), 'alt':(7000, 10001), 'ground_speed':(196, 387),
                    'vrate': (-2112, -181), 'ground_distance': (0, 0.125), 'name': 'SERFR'}
...
def apply_route_conditions(cond_set, row):
    ...
    if float(int(track)) in range(track_min, track_max) and int(altitude) in range(alt_min, alt_max) and \
            int(vertical_rate) in range(vrate_min, vrate_max) and int(ground_speed) in range(ground_speed_min, ground_speed_max)\
            and float(ground_distance) >= ground_distance_min and float(ground_distance) <= ground_distance_max:
...
# in order to complete the classifcation task that it is given.
# ----------------------------------------------------------------------------------------
def sightingReader():
